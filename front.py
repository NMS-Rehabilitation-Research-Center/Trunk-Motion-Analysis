"""Desktop video tool for real-time trunk flexion and extension feedback."""
...
class SpineAnalysisApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Trunk Motion Analysis - Flexion / Extension")
        ...
        self.forward_choice = tk.StringVar(value="Image left")
    ...
    def create_widgets(self):
        ...
        info_frame = ttk.LabelFrame(main_frame, text="Trunk Motion Analysis", padding=15)
        ...
        ttk.Label(angle_frame, text="Trunk angle:", font=('Arial', 12)).pack(side=tk.LEFT)
        ...
        ttk.Label(flex_frame, text="Flexion:", font=('Arial', 11)).pack(side=tk.LEFT)
        self.flexion_label = ttk.Label(flex_frame, text="0°", font=('Arial', 16, 'bold'), foreground='#00ccff')
        self.flexion_label.pack(side=tk.LEFT, padx=10)
        
        ext_frame = ttk.Frame(info_frame)
        ext_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(ext_frame, text="Hyperextension:", font=('Arial', 11)).pack(side=tk.LEFT)
        self.extension_label = ttk.Label(ext_frame, text="0°", font=('Arial', 16, 'bold'), foreground='#ff6b6b')
        self.extension_label.pack(side=tk.LEFT, padx=10)
        ...
        ttk.Label(max_flex_frame, text="Peak flexion:", font=('Arial', 10)).pack(side=tk.LEFT)
        self.max_flex_label = ttk.Label(max_flex_frame, text="0°", font=('Arial', 11, 'bold'), foreground='#00ccff')
        self.max_flex_label.pack(side=tk.LEFT, padx=10)
        
        max_ext_frame = ttk.Frame(max_frame)
        max_ext_frame.pack(fill=tk.X, pady=2)
        ttk.Label(max_ext_frame, text="Peak hyperextension:", font=('Arial', 10)).pack(side=tk.LEFT)
        self.max_ext_label = ttk.Label(max_ext_frame, text="0°", font=('Arial', 11, 'bold'), foreground='#ff6b6b')
        self.max_ext_label.pack(side=tk.LEFT, padx=10)
        ...
        ttk.Label(control_frame, text="Forward:").pack(side=tk.LEFT, padx=(12, 4))
        self.forward_menu = ttk.Combobox(
            control_frame,
            textvariable=self.forward_choice,
            values=("Image left", "Image right"),
            state="readonly",
            width=10,
        )
        self.forward_menu.pack(side=tk.LEFT)
        self.forward_menu.bind("<<ComboboxSelected>>", self.on_forward_changed)
    ...
    def on_forward_changed(self, _event=None):
        """Set which side of the displayed sagittal view is forward."""
        self.forward_factor = 1 if self.forward_choice.get() == "Image left" else -1
    ...
    def reset_data(self):
        ...
        self.max_flex_label.config(text="0°")
        self.max_ext_label.config(text="0°")
    ...
    def process_video(self):
        ...
        while self.is_playing and not self.stop_analysis:
            ...
            self.analysis_data.append({
                ...
                "Flexion (deg)": f"{self.flexion:.2f}" if self.pose_detected else "",
                "Hyperextension (deg)": f"{self.extension:.2f}" if self.pose_detected else "",
                "Pose detected": "Yes" if self.pose_detected else "No",
                "Rotation (deg)": self.rotation_degrees,
                "Forward direction": self.forward_choice.get(),
            })
    ...
    def calculate_spine_angle(self, landmarks, w, h):
        """Calculate trunk flexion and extension from pose landmarks."""
        # Required landmarks
        ...
        # Directed image-plane angle: upright is 180 degrees. The selected
        # forward side determines which direction is flexion.
        if np.linalg.norm(spine_vec) > 0:
            signed_tilt = np.degrees(np.arctan2(spine_vec[0], -spine_vec[1]))
            self.spine_angle = (180 + signed_tilt * self.forward_factor) % 360
        else:
            self.spine_angle = 180
        
        if self.spine_angle < 180:
            self.flexion = 180 - self.spine_angle
            self.extension = 0
        elif self.spine_angle > 180:
            self.flexion = 0
            self.extension = self.spine_angle - 180
        else:
            self.flexion = 0
            self.extension = 0
        
        # Peak values
        if self.flexion > self.flexion_max:
            self.flexion_max = self.flexion
        if self.extension > self.extension_max:
            self.extension_max = self.extension
    ...
    def update_gui(self):
        ...
        self.flexion_label.config(text=f"{int(self.flexion)}°")
        self.extension_label.config(text=f"{int(self.extension)}°")
        
        self.max_flex_label.config(text=f"{int(self.flexion_max)}°")
        self.max_ext_label.config(text=f"{int(self.extension_max)}°")
        
        self.rom_label.config(text=f"{int(self.rom_min)}° - {int(self.rom_max)}°")
        
        if self.flexion > 30:
            self.status_label.config(text="High flexion", foreground='#ff6b6b')
        elif self.extension > 30:
            self.status_label.config(text="High hyperextension", foreground='#ff6b6b')
        elif self.flexion > 15:
            self.status_label.config(text="Moderate flexion", foreground='#ffdd00')
        elif self.extension > 15:
            self.status_label.config(text="Moderate hyperextension", foreground='#b8860b')
        else:
    ...
    def update_chart(self):
        ...
        self.chart_canvas.create_text(width//2, 5, text="Flexion  |  Hyperextension",
    ...
    def draw_info_on_frame(self, frame):
        ...
        y = 30
        cv2.putText(frame, "TRUNK ANALYSIS", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        y += 25
        
        cv2.putText(frame, f"Angle: {int(self.spine_angle)} deg", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        y += 20
        
        cv2.putText(frame, f"Flexion: {int(self.flexion)} deg", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 1)
        y += 20
        
        cv2.putText(frame, f"Hyperextension: {int(self.extension)} deg", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 100, 100), 1)
        y += 20
        ...
        if self.flexion > 30:
            ...
            status = "High hyperextension"
            color = (0, 0, 255)
        elif self.flexion > 15:
            status = "Moderate flexion"
            color = (0, 255, 255)
        elif self.extension > 15:
            status = "Moderate hyperextension"
            color = (0, 255, 255)
        ...
        if self.flexion > 5:
            ...
            cv2.putText(frame, "FLEXION", (w-120, h//2+30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 200, 255), 1)
        ...
        if self.extension > 5:
            ...
            cv2.putText(frame, "HYPEREXTENSION", (w-175, h//2+30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 100, 100), 1)
    ...
    def export_data(self):
        ...
        default_name = "trunk_analysis.csv"
        if self.video_path:
            video_name = self.video_path.rsplit('/', 1)[-1].rsplit('.', 1)[0]
            default_name = f"{video_name}_trunk_analysis.csv"
